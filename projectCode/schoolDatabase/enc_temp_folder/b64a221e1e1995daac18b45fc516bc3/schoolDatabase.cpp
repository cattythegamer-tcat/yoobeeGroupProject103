...
void loadSchool() {
    ...
    while (std::getline(teacherAccounts, teacherInfo)) {
        ...
        for (char& c : teacherInfo) {
            if (c == ',') {
                dataPos++;
                if (dataPos == 2 || 9) teachers.push_back("");
                continue;
            }
            else if (dataPos == 2 || 9) teachers[teachers.size() - 1] += c;
        }
    }
    teacherAccounts.close();
    for (int teachIdx = 0; teachIdx < teachers.size(); teachIdx++) {
        cout << teachers[teachIdx] << endl;
    }
}
